...
class NewBeerEnroll extends Component {
    ...
    getStores() {
        executeQuery({
            ...
            success: (res) => {
                let result = [];
                const resDoc = res.users || [];
                _.map( resDoc, ( docItem, docIndex ) => {
                    result.push({
                        value: docItem.id,
                        title: docItem.storeName,
                    });
                });
                this.setState({
                    stores: result
    ...
    handleSubmitBtn = () => {
        ...
        if (!wine.uid) {
            pushNotification(NOTIFICATION_TYPE_ERROR, '본사를 선택해주세요');
            return;
        }
        if (!wine.name) {
    ...
    render() {
        const { pMode, pId, user } = this.props;
        const { sIsVisible, sUserId, sUnitPrice, sUserImage, sUserImageFromFileUpload, dangdo, sando, badi, tanin, wine, stores } = this.state;
                ...
                >
                    ...
                    <ModalBody className='modal-body'>
                        <div className='row form-group'>
                            <label className="control-label col-sm-2">본사</label>
                            <select className='form-control col-sm-10' value={wine.uid} name='uid' onChange={(e) => {this.handleInfoChange(e)}}>
                                <option></option>
                                {
                                    stores.map((store, index) => (
                                        <option key={index} value={store.value}>{store.title}</option>
                                    ))
                                }
                            </select>
                        </div>
